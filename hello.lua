...
local Theme = {
    Primary = Color3.fromRGB(21, 103, 251),
    Secondary = Color3.fromRGB(95, 95, 95),
    Background = Color3.fromRGB(255, 255, 255),
    BackgroundTransparency = 0.15,
    Text = Color3.fromRGB(95, 95, 95),
...
--[[
    @description Creates a new window with the specified parameters
    @param ti string Title text for the window
    @param dosplash boolean Whether to show splash screen
    @param visiblekey KeyCode Key to toggle window visibility
    @param deleteprevious boolean Whether to delete previous UI
...
function lib:init(ti, dosplash, visiblekey, deleteprevious)
    if syn then
        ...
        if cg:FindFirstChild("ScreenGui") and deleteprevious then
           CreateTween(cg.ScreenGui.main, {Position = cg.ScreenGui.main.Position + UDim2.new(0,0,2,0)}, 0.5):Play()
    ...
    elseif gethui then
        if gethui():FindFirstChild("ScreenGui") and deleteprevious then
            CreateTween(gethui().ScreenGui.main, {Position = gethui().ScreenGui.main.Position + UDim2.new(0,0,2,0)}, "InOut", "Quart", 0.5):Play()
    ...
    if dosplash then
        ...
        CreateTween(splash, {Position = UDim2.new(0.5, 0, 0.5, 0)}, "InOut", "Quart", 1):Play()
        wait(2)
        CreateTween(splash, {Position = UDim2.new(0.5, 0, 2, 0)}, "InOut", "Quart", 1):Play()
    ...
    local function constrainToBounds(position)
        local viewportSize = workspace.CurrentCamera.ViewportSize
        local mainSize = main.AbsoluteSize
        
        return Vector2.new(
            math.clamp(position.X, 0, viewportSize.X - mainSize.X),
            math.clamp(position.Y, 0, viewportSize.Y - mainSize.Y)
        )
    end
    ...
    local function update(input)
        ...
        local constrainedPos = constrainToBounds(targetPos)
        main.Position = UDim2.new(0, constrainedPos.X, 0, constrainedPos.Y)
    end
    ...
    close.MouseButton1Click:Connect(function()
        for _, callback in ipairs(Events.OnWindowClose) do
            window:SafeCallback(callback)
        end
        scrgui:Destroy()
    end)
    ...
    if ti then
       ...
       CreateTween(main, {Position = UDim2.new(0.5, 0, 0.5, 0)}, 1):Play()
    window = {}
    ...
    function window:ToggleVisible()
        ...
        dbcooper = true
        if visible then
            CreateTween(main, {Position = UDim2.new(0.5, 0, 0.5, 0)}, 0.5):Play()
            task.wait(0.5)
            dbcooper = false
        else
            CreateTween(main, {Position = main.Position + UDim2.new(0,0,2,0)}, 0.5):Play()
            task.wait(0.5)
            dbcooper = false
        end
    end
    ...
    function window:SafeCallback(callback, ...)
        ...
        if not success then
            window:TempNotify(
                "Error", 
                "Callback error: " .. tostring(result),
                "rbxassetid://12608260095" -- Error icon
            )
        end
    ...
    local function updateUIScale()
        ...
        main.Size = UDim2.new(0, 721 * scale, 0, 584 * scale)
    end
